...
class Gradients:
    ...
    def compute(self, weights, algos, activations, target):
        """
        Compute gradients
        ...
        calculator = GradientCalculator(algos[-1], activations[-1], target)
        for layer in reversed(range(num_layers)):
            ...
            calculator.backpropagation(layer, weights, algos, activations)

class GradientCalculator:
    def __init__(self, algo, activation, target):
        # start derivatives at output edge with target
        if algo == "softmax":
            cost_wrt_output = func.cross_entropy_loss(activation, target)
        else:
            cost_wrt_output = func.mean_squared_error_derivative(activation, target)
        pre_activation_wrt_activation = self.derivative_with_algo(algo, activation, target)
        self.cost_wrt_activation = cost_wrt_output * pre_activation_wrt_activation

    @classmethod
    def derivative_with_algo(cls, algo, activation, target=None):
        """
        ...
        :param activation: Activated NumPy array
        :param target: Target NumPy array (for last layer only)
        :return: Derivative of the activation of output NumPy array.
        """
        if algo == "sigmoid":
            return func.sigmoid_derivative(activation)
        elif algo == "relu":
            return func.relu_derivative(activation)
        elif algo == "tanh":
            return func.tanh_derivative(activation)
        elif algo == "softmax" and target is not None:
            return func.cross_entropy_gradient(activation, target)
        else:
            raise ValueError(f"Unsupported derivative of activation algorithm: {algo}")

    def backpropagation(self, layer, weights, algos, activations):
        """
        ...
        if layer > 0:
            ...
            prev_algo = algos[layer - 1]
            pre_activation_wrt_activation = self.derivative_with_algo(prev_algo, activation)
            ...
            self.cost_wrt_activation = prev_layer_weighted_error * pre_activation_wrt_activation
